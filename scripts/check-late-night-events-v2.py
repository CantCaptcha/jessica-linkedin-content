...
def main():
    events = check_late_night_events()
    ...
    print(f"[DEBUG] Checking dates: {', '.join(dates_to_check)}")
    print(f"[DEBUG] Found {len(events)} matching events")
    
    if events:
        for event in events:
            event_start = event['start'].astimezone(timezone.utc).astimezone()
            print(f"[DEBUG] Event: {event['summary']} | UTC: {event['start'].strftime('%Y-%m-%d %H:%M')} | Local: {event_start.strftime('%Y-%m-%d %H:%M')}")
